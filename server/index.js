...
function isCommandPreviewOnlyTask(text) {
  const normalized = String(text || '').replace(/\r/g, '').trim();
  if (!normalized || normalized.startsWith('[')) return false;

  if (!/(命令|指令|shell\s*命令|bash\s*命令)/i.test(normalized)) return false;

  const hasPreviewIntent = /(输出|给我|告诉我|显示|列出|提供|返回|贴出|写出|发我|只要|只输出|只显示|不要执行|不要运行|别执行|别运行|怎么|如何|什么命令|命令是什么)/.test(normalized)
    || /(?:查看|查询|检查|获取|显示|输出|列出|打印|看下).{0,30}的命令/.test(normalized)
    || /(?:怎么|如何).{0,20}(查看|查询|检查|获取|显示|输出)/.test(normalized);
  if (!hasPreviewIntent) return false;

  if (/(帮我执行|帮我运行|直接执行|直接运行|自动执行|自动运行|立即执行|马上执行)/.test(normalized)
    && !/(不要执行|不要运行|别执行|别运行|只输出|只显示)/.test(normalized)) {
    return false;
  }
...
function normalizeCommandPreviewReply(text) {
  let normalized = String(text || '').trim();
  if (!normalized) return '';

  normalized = normalized.replace(/<command(?:\s+[^>]*)?>([\s\S]*?)<\/command>/gi, (_, cmd) => {
    const value = String(cmd || '').trim();
    return value ? '\n\n```bash\n' + value + '\n```\n\n' : '';
  });

  normalized = normalized.replace(/<mcp-call(?:\s+[^>]*)?>[\s\S]*?<\/mcp-call>/gi, '').trim();

  if (!/```[a-zA-Z]*/.test(normalized)) {
    const extracted = extractCommandFromText(normalized);
    if (extracted) {
...
function detectReferenceOnlyCommandContext(text) {
  const normalized = String(text || '').replace(/\r/g, '').trim();
  if (!normalized || normalized.startsWith('[')) return false;

  const hasReferenceMarker = /(参考下|参考这个|参考如下|参考以下|仅参考|作为参考|示例|例如|比如|下面.*参考|以下.*参考|其中以下部分.*参考|不要直接执行|不要照抄执行|以项目的?.*为准|以当前项目.*为准|以.*Dockerfile.*为准)/.test(normalized);
  if (!hasReferenceMarker) return false;
  ...
  for (const line of lines) {
    ...
    if (classifyInlineInput(plain) === 'shell') commandLikeLines += 1;
  }

  return commandLikeLines >= 2;
}

function decorateUserMessageForPlanning(text) {
  const normalized = String(text || '');
  if (!detectReferenceOnlyCommandContext(normalized)) return normalized;

  return `[系统提示: 用户消息中的多行 shell 命令、Dockerfile 片段、配置片段若被描述为“参考/示例/如下内容/以项目为准”，默认只作为上下文，不得直接当成下一步要执行的命令。请以当前项目实际文件和用户最终目标为准，自行判断真正需要执行的下一条命令。只有当用户明确要求“按这些命令执行/照着执行”时，才能直接采用参考块中的命令。]\n${normalized}`;
}
...
wss.on('connection', (ws) => {
  ...
  function createAITask(rootUserMessage = '') {
    return {
      ...
      responseMode: isCommandPreviewOnlyTask(rootUserMessage) ? 'command_preview' : 'execute',
    };
  }
  ...
  function buildSystemPrompt(responseMode = 'execute') {
...
你的职责：理解用户自然语言指令，给出简洁中文分析，转化为可执行 shell 命令，分析执行结果。`;
    ...
    if (responseMode === 'command_preview') {
      p += `

当前请求属于【命令展示模式】：用户是想知道“该用什么命令”，不是要你直接执行。

严格要求：
- 只回答命令建议，不要进入执行流程。
- 严禁输出 <command>...</command> 或 <mcp-call>...</mcp-call>。
- 不要写“请执行”“现在执行”“下一步执行”。
- 优先给出 1 条最直接的命令；如确有必要，最多补充 1 条备选。
- 使用 markdown 的 bash 代码块输出命令。
- 可以先用一句中文说明用途，但要非常简洁。
- 不要假设你已经执行了命令，也不要分析不存在的执行结果。`;
    } else {
      p += `
...
【参考内容识别规则】用户有时会把“背景参考材料”和“任务指令”一起粘贴进来，必须正确理解语境。

关键认知：当用户说“参考下以下内容”/“参考下这个”/“参考一下”后跟着换行内容时，用户是在**给你提供参考材料**，不是在要求你去提供参考资料。这是用户在喂给你背景上下文，紧接着后面或者之前的中文指令才是真正要执行的任务。

禁止行为：用户提供了参考块 + 任务指令时，严禁回复“好的，请提供您需要我参考的内容”或“请说明具体任务”——参考内容和任务都已经在消息里了。

正确做法：把“参考下以下内容……”后面的内容视为背景上下文，把消息中的中文自然语言指令（如“以项目的 Dockerfile 为准，打出一个新的镜像来”）视为任务，直接开始执行，参考块作为背景信息辅助决策。
...
【交互式命令说明】docker exec -it、ssh、su、vim、htop 等需要终端交互的命令可以正常输出，系统会直接写入终端让用户接管，不会捕获输出。`;
  ...
  async function handleAITurn(userMessage, task = null, options = {}) {
    ...
    if (!task) sendLog(`handleAITurn 新任务 execModeOverride=${options.execModeOverride || 'none'}`, 'info');
    ...
    const responseMode = aiTask.responseMode || 'execute';

    if (isRootTurn) {
      activeAITask = aiTask;
      send('ai_task_start');
      if (responseMode === 'command_preview') {
        sendLog('检测为命令展示请求：仅返回命令文本，不生成确认执行卡片', 'info');
      }
    }
    ...
    const plannerUserMessage = decorateUserMessageForPlanning(userMessage);
    aiHistory.push({ role: 'user', content: plannerUserMessage });
    ...
    try {
      ...
      let firstChunk = true;
      for await (const chunk of streamAI(buildSystemPrompt(responseMode), aiHistory, signal)) {
        if (firstChunk) { sendLog(`AI 开始输出...`, 'step'); firstChunk = false; }
        if (responseMode === 'command_preview') {
          if (isAITaskCancelled(aiTask, signal)) break;
          fullReply += chunk;
          continue;
        }
      ...
      if (responseMode === 'command_preview') {
        ...
        const previewReply = normalizeCommandPreviewReply(fullReply);
        if (previewReply) send('ai_reply_chunk', { text: previewReply });
        sendLog('AI 回复完成 (命令展示，不执行)', 'ok');
        send('ai_reply_end');
        aiHistory.push({ role: 'assistant', content: previewReply || fullReply });
        return;
      }
  ...
  ws.on('message', async (raw) => {
    ...
    switch (type) {
      ...
      case 'update_host_context': {
        ...
        if (payload.execModeOverride === null) {
          if (activeAITask) activeAITask.execModeOverride = null;
        } else if (['ask_each', 'auto_approve_low', 'auto_approve_all'].includes(payload.execModeOverride)) {
          if (activeAITask) activeAITask.execModeOverride = payload.execModeOverride;
        }
        break;
      }
      ...
      case 'input': {
        ...
        if (kind === 'natural') {
          ...
          sendLog(`自然语言模式 → 交由 AI 处理 [execModeOverride=${execModeOverride || 'none'}]`, 'step');
